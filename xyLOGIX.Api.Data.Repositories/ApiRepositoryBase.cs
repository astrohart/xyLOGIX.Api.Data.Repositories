...
{
///  <summary> Provides functionality that is common to all REST API repository objects. </summary> <typeparam name="T"> Name -- should be that of a concrete type -- of the type of a POCO that specifies a single element of the collection exposed by this repository. <para /> An interface name may be used; however, implementers must then adapt their methods' outputs to the interface specified, in this case. <para /> Implementers and clients are cautioned that if this type parameter is supplied with an interface, further generic abstraction may be required in other areas of the software system. </typeparam> <remarks> This class provides access to a paginated/iterated REST API data set, of an a priori unknown length, as if it were, e.g., a SQL Server data source. Basically, we try to merge the concepts of data retrieval from a REST API data set and the concepts of the, e.g., Repository pattern in the N-Tier data access world. <para /> Implementers are free to deny access to specific functionality by throwing <see cref="T:System.NotSupportedException" /> for any of the methods this interface exposes, given varying target REST API use-case and support scenarios. </remarks>
    public abstract class ApiRepositoryBase<T> : IApiRepository<T>
        where T : class
    {
///  <summary> Reference to an instance of an object that implements the <see cref="T:xyLOGIX.Api.Data.Iterables.Interfaces.IIterable{T}" /> interface. This objects provides us with data. It's kind of like the DbContext field we utilize in a Repository class that is used in Entity Framework. </summary>
        private IIterable<T> _iterable;

///  <summary> Constructs a new instance of <see cref="T:xyLOGIX.Api.Data.Repositories.Interfaces.ApiRepositoryBase" /> and returns a reference to it. </summary> <param name="pageSize"> (Optional.) Integer value specifying the number of elements to obtain at a time while iterating over the collection supplied by the target REST API. <para /> The default value of this parameter is one. </param> <param name="maxPageSize"> (Required.) Integer value specifying the maximum number of entries the API will fetch in a single method call. This varies depending on the particular REST API in use, therefore this is a constructor parameter. </param> <exception cref="T:System.ArgumentNullException"> Thrown if the required parameter, <paramref name="iterable" />, is passed a <c>null</c> value. </exception> <exception cref="T:System.ArgumentOutOfRangeException"> Thrown if the <paramref name="pageSize" /> parameter is zero or negative, or if the <paramref name="maxPageSize" /> is less than 1. </exception>
...
///  <summary> Gets or sets the maximum number of elements per page that the API will allow to be fetched. </summary> <remarks> This is an abstract property because this quantity is different for every target REST API. </remarks>
...
///  <summary> Associates this repository with a data source. </summary> <param name="iterable"> (Required.) Reference to an instance of a class that implements the <see cref="T:xyLOGIX.Api.Data.Iterables.Interfaces.IIterable" /> interface to which to associate this repository. </param> <returns> </returns>
...
///  <summary> Iterates through the dataset of the target REST API, <see cref="P:xyLOGIX.Api.Data.Repositories.Interfaces.IApiRepository.PageSize" /> elements at a time (default is 1), and tries to find an element matching the criteria provided. </summary> <param name="predicate"> (Required.) Lambda expression specifying how to tell if an element is to be retrieved. </param> <returns> This method iterates through the dataset of the target REST API, testing each element against the provided <paramref name="predicate" /> . The first element for which the <paramref name="predicate" /> evaluates to <c>true</c> is then returned, or <c>null</c> if an error occurred or the matching element was otherwise not found. </returns> Reference to an instance of an object of type <typeparamref name="T" /> if an object matching the criteria is found; <c> null </c> otherwise. <remarks> Clients of this repository should use this method instead of invoking the GetAll operation and then filtering with the LINQ Where method, in order to retrieve just those API elements that need to be retrieved until the desired one is found. <para /> GetAll will suck down the entire dataset, and this may not be desirable because of rate limits etc. <para /> Implementations should throw <see cref="T:System.NotSupportedException" /> in the event that the API does not support pagination -- or delegate the call to this object's GetAll followed by <see cref="M:System.Linq.Enumerable.Where" /> followed by <see cref="M:System.Linq.Enumerable.FirstOrDefault" />. <para /> Alternatively, implementers may delegate this method to <see cref="M:xyLOGIX.Api.Data.Repositories.Interfaces.IApiRepository.Get" />. <para /> This repository provides these two seemingly redundant ways of searching for objects since not all REST API controllers expose the same functionality set or have the same rate-limit concerns. <para /> <strong>EXTREME CAUTION</strong> IF rate-limiting and data set size are concerns, then it is probably better to call the <see cref="M:xyLOGIX.Api.Data.Repositories.Interfaces.IApiRepository.Get" /> method. This passes the search criteria directly to the target REST API server and harnesses the server's own power to perform the search for the desired item, instead of going through the entire data set, one by one, until a match is found. </remarks> <exception cref="T:System.Exception"> Bubbled up from whichever method call is made on the library that accesses the target REST API in the event the operation was not successful. </exception> <exception cref="T:ArgumentNullException"> Thrown if the required parameter, <paramref name="predicate" /> , is passed a <c>null</c> value. </exception> <exception cref="T:System.NotSupportedException"> Might be if the target API does not support the concept of pagination. </exception>
...
///  <summary> Raises the <see cref="E:xyLOGIX.Api.Data.Iterators.IteratorBase.IterationError" /> event. </summary> <param name="e"> A <see cref="T:xyLOGIX.Api.Data.Iterators.Events.IterationErrorEventArgs" /> that contains the event data. </param>
